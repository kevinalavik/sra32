...
static void sra32_set_flags_add(cpu_t *cpu, uint32_t a, uint32_t b, uint32_t r)
{
    ...
    if (r < a)
        cpu->flags |= FLAG_C; // unsigned carry out
    if (~(a ^ b) & (a ^ r) & 0x80000000)
        cpu->flags |= FLAG_O; // signed overflow
}
...
static uint32_t sra32_alu(cpu_t *cpu, uint8_t funct, uint32_t a, uint32_t b, const sra32_instr_t *in)
{
    ...
    case ALU_ADD:
        ret = a + b;
        sra32_set_flags_add(cpu, a, b, ret);
        return ret;
    case ALU_SUB:
        ret = a - b;
        sra32_set_flags_sub(cpu, a, b, ret);
        return ret;
    case ALU_MUL:
        ret = a * b;
        break;
    ...
    case ALU_SDIV:
        if (b == 0)
            ret = 0xFFFFFFFF; // div by zero -> -1
        else if (a == 0x80000000 && b == 0xFFFFFFFF)
            ret = 0x80000000; // INT_MIN / -1 (i think)
        else
            ret = (uint32_t)((int32_t)a / (int32_t)b);
        break;
    case ALU_UDIV:
        ret = (b == 0) ? 0xFFFFFFFF : a / b;
        break;
    case ALU_AND:
        ret = a & b;
        break;
...
void cpu_handle_load(cpu_t *cpu, const sra32_instr_t *in)
{
    ...
    {
    case MEM_BYTE:
        cpu->regs[in->rd] = cpu->read(cpu, addr, 1) & 0xFF;
        break;
    case MEM_HALF:
        cpu->regs[in->rd] = cpu->read(cpu, addr, 2) & 0xFFFF;
        break;
    case MEM_WORD:
        cpu->regs[in->rd] = cpu->read(cpu, addr, 4);
        break;
    default:
        sra32_trap(cpu, SRA32_TRAP_ILLEGAL, in->word);
        break;
...
void cpu_handle_store(cpu_t *cpu, const sra32_instr_t *in)
{
    ...
    {
    case MEM_BYTE:
        cpu->write(cpu, addr, cpu->regs[in->rd], 1);
        break;
    case MEM_HALF:
        cpu->write(cpu, addr, cpu->regs[in->rd], 2);
        break;
    case MEM_WORD:
        cpu->write(cpu, addr, cpu->regs[in->rd], 4);
        break;
    default:
        sra32_trap(cpu, SRA32_TRAP_ILLEGAL, in->word);
        break;
...
{
    cpu->regs[in->rd] = (cpu->regs[in->rd] & 0xFFF) | (in->imm << 12);
}
...
void cpu_handle_br(cpu_t *cpu, const sra32_instr_t *in)
{
    ...
    if (taken)
        cpu->pc = in->pc + ((uint32_t)in->simm << 2);
}
